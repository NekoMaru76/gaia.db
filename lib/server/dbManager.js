  ...
  const db = {
    ...
    async getDBKeyPath(user, dbName, keyName) {
      try {
        ...
        if (typeof dbPath === "number") throw dbPath;
    ...
    async getDBKeyID(user, dbName, keyName) {
      ...
      try {
        ...
        for (const check of [dbPath, dbData, dataPath])
          if (typeof check === "number") throw check;
        
        if (!Object.values(dbData.datas).includes(keyName)) throw 412;
    ...
    async setKey(user, dbName, keyName, keyValue, keyPath) {
      ...
      try {
        ...
        if (typeof dbPath !== 200 && typeof dbPath === "number") throw dbPath;
        if (typeof dbData === "number") throw dbData;
    ...
    async createKey(user, dbName, keyName, keyValue) {
      ...
      try {
        ...
        if (typeof mainData === "number") throw mainData;
        if (Object.values(mainData).includes(keyName)) throw 411;
        ...
        for (const check of r) if (typeof check === "number" && check !== 200) throw check;
