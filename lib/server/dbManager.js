...
async function DBManager() {
	const db = {
		...
		async getDBKeyPath(user, dbName, keyName) {
			for (const check of [user, dbName, keyName]) if (typeof check !== "string") return 401;
		...
		async getDBFolder(user, name) {
			for (const check of [user, name]) if (typeof check !== "string") return 401;
		...
		async getKey(user, dbName, keyName, keyPath) {
			for (const check of [user, dbName, keyName]) if (typeof check !== "string") throw 401;
		...
		async createUser(user, password) {
			for (const check of [user, password]) if (typeof check !== "string") return 401;
